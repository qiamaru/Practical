...
import com.example.demoMirul.model.*;
...
public class ItemService {
	...
	public Optional<Item> updateItem(Long id, String newValue) {
		...
		// The lambda function returns the newValue, which will replace the existing
		// value.
		// If the key is not present, the lambda is not executed, and computeIfPresent
		// returns null.
		String updatedValue = Data.getDataStore().computeIfPresent(id, (key, existingValue) -> newValue);

		if (updatedValue != null) {
			// If updatedValue is not null, it means the key was present and the value was
			// updated.
